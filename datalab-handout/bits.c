...
int isLessOrEqual(int x, int y) {
  int signOfX = (x >> 31) & 0x01;
  int signOfY = (y >> 31) & 0x01;
  int xPyN = signOfX & (~signOfY);
  int xNyP = (~signOfX) & signOfY;
  int result = y + ((~x)+1);
  return (!xNyP) & (xPyN | !(result >> 31)) ;
}
//4
/* 
